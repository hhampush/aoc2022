...
void parse_dirtree()
{
    ...
    while (ptr != NULL)
    {
        ...
        {
            uint dir_hash = hash(current_path);
            char dir_name = '\0';
            uint file_size = 0;
            if (sscanf(ptr, "dir %c", &dir_name) == 1)
            {
                ;
            }
            else if (sscanf(ptr, "%u", &file_size) == 1)
            {
...
int main()
{
    ...
    ulint const file_size_root = file_size_per_dir[SPECIAL_ROOT_INDEX];
    ulint sum = 0;
    ...
    for (uint i = 0; i < TABLE_SIZE; i++)
    {
        ...
        if (part_two)
        {
            uint const unused = 70000000 - file_size_root;
